...
class DataManager:
    ...
    def addgame(self, name: str) -> bool:
        if name.lower() not in [g.lower() for g in self.games]:
            self._data["games"].append(name)
            self.save()
            return True
        return False

    def removegame(self, name: str) -> bool:
        for g in self.games:
            if g.lower() == name.lower():
                self._data["games"].remove(g)
                self.save()
                return True
        return False
    ...
    def add_yes_participant(self, user_id: int):
        yes = set(self._data.get("yes_participants", []))
        no = set(self._data.get("no_participants", []))
        yes.add(user_id)
        no.discard(user_id)
        self._data["yes_participants"] = list(yes)
        self._data["no_participants"] = list(no)
        self.save()
